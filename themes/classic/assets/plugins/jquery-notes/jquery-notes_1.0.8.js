...
(function ($) {
    ...
    $.fn.jQueryNotes = function (settings) {
        ...
        var _getNotes = function (pointer) {
            ...
            if (settings.loadNotes) {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    beforeSend: function () {
                        (ID.firstLoad) ? _startLoading(pointer, 'Загрузка...') : '';
                    },
                    ...
                    success: function (data) {
                        ...
                        if (data) {
                            ...
                            $('#jquery-notes_' + pointer + ' .controller .counter').attr('title', function () {
                                return (counter == 1) ? counter + ' отметка' : counter + ' отметок';
                            });
        ...
        var _printNote = function (pointer, note) {
            ...
            var html = '<div id="n_' + pointer + '-' + note.ID + '" class="note"><div class="border"><i class="glow glow_' + note.AVATAR + '">';
            ...
            if (position.width < 20) {
                var delta = 20;
            } else if (position.width > 20 && position.width < 40) {
                delta = 10;
            } else {
                delta = 0;
            }
        ...
        var _addNote = function (pointer, event) {
            ...
            if (ID.add && !ID.set) {
                ...
                $('#jquery-notes_' + pointer + ' .notes').append('' +
                    ...
                    '<div class="border">' +
                    '<a href="javascript:void(0)" class="glow_switch up"><i class="icon icon-arrow_small_top_white"></i></a>' +
                    '<a class="glow_current" href="javascript:void(0)" title="Аватар"><i data-avatar="1" class="glow-small glow-small_1"></i></a>' +
                    '<a href="javascript:void(0)" class="glow_switch down"><i class="icon icon-arrow_small_bottom_white"></i></a>' +
                    '</div>' +
                    '</div>');
        ...
        var _saveNote = function (pointer, id, operation) {
            ...
            avatar = $('a.glow_current i').attr('data-avatar');
            ...
            $.ajax({
                ...
                data: operation + '=true&image=' + ID.image + '&id=' + id + '&position=' + position.left + ',' + position.top + ',' + position.width + ',' + position.height + '&note=' + note + '&link=' + link + '&avatar=' + avatar,
                success: function (data) {
                    ...
                    if (data) {
                        ...
                        alert('Не получилось сохранить отметку');
        ...
        var _openSettings = function (pointer, id) {
            ...
            if ((settings.allowEdit || settings.allowDelete) && !ID.edit && !ID.add) {
                ...
                html += '<a href="javascript:void(0);" class="cancel-note" title="Отменить" />';
